...
- Index file format is versioned (`{"version": 1, "root": ..., "raw": {...}}`). Bump `INDEX_VERSION` if the schema changes. On version mismatch, `load` raises a clear error telling the user to re-run `build`. No migration shims — old indexes are invalid, not migrateable (see Corollary 1.3 / 4.3 in CONSTITUTION.md).
